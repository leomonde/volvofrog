...
def get_stopped_equivalence_factor(v_ego, v_lead, v_lead_distance, t_follow, aggressive_acceleration):
  distance_offset = 0
  ...
  if aggressive_acceleration and np.all(speed_difference <= -1):
    ...
    offset = np.clip(np.abs(speed_difference) - v_ego, 0, 10)
    distance_offset += np.clip(v_lead_distance, 0, offset)
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard):
    ...
    lead_0_obstacle = lead_xv_0[:,0] + get_stopped_equivalence_factor(self.x_sol[:,1], lead_xv_0[:,1], lead_xv_0[:,0], t_follow, self.aggressive_acceleration)
    lead_1_obstacle = lead_xv_1[:,0] + get_stopped_equivalence_factor(self.x_sol[:,1], lead_xv_1[:,1], lead_xv_1[:,0], t_follow, self.aggressive_acceleration)
