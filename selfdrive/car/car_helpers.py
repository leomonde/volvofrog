...
def format_params(params):
  return [f"{k}: {v.decode('utf-8') if isinstance(v, bytes) else v}" for k, v in params.items()]

def get_frogpilot_params(params, keys):
  return {key: params.get(key, '0') for key in keys}
...
def crash_log(candidate):
  params = Params()
  control_keys, visual_keys = (
    "AlwaysOnLateral", "ConditionalExperimentalMode", "ConditionalExperimentalModeSpeed", "ConditionalExperimentalModeSpeedLead", 
    ...
    "StandardPersonalityValue", "StandardJerkValue", "RelaxedPersonalityValue", "RelaxedJerkValue", "DeviceShutdownTimer", 
    "DrivingPersonalitiesUIWheel", "ExperimentalModeViaWheel", "FireTheBabysitter", "DisableAllLogging", "MuteDM", 
    ...
    "LaneChangeTimer", "LaneDetection", "OneLaneChange", "TurnDesires", "DisableInternetCheck", "TwilsoncoSSH"
  ), (
    "FrogTheme", "FrogColors", "FrogIcons", "FrogSignals", "FrogSounds", "Compass", "CustomRoadUI", 
    "LaneLinesWidth", "RoadEdgesWidth", "PathWidth", "PathEdgeWidth", "AccelerationPath", "BlindSpotPath", "UnlimitedLength", "DeveloperUI", 
    "NumericalTemp", "Fahrenheit", "RotatingWheel", "ScreenBrightness", "Sidebar", "SilentMode", "SteeringWheel", "WideCameraDisable",
    "HideSpeed"
  )

  control_params, visual_params = map(lambda keys: get_frogpilot_params(params, keys), [control_keys, visual_keys])
  control_values, visual_values = map(format_params, [control_params, visual_params])
  control_chunks, visual_chunks = map(lambda data: chunk_data(data, 50), [control_values, visual_values])

  with sentry_sdk.configure_scope() as scope:
    for chunks, label in zip([control_chunks, visual_chunks], ["FrogPilot Controls", "FrogPilot Visuals"]):
